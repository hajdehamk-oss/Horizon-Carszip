...
router.post("/orders", requireAdminAuth, async (req, res, next) => {
  try {
...
// Admin: delete order
router.delete("/orders/:id", requireAdminAuth, async (req, res, next) => {
  try {
    const id = Number(req.params.id);
...
router.post("/orders/request", requireClientAuth, async (req, res, next) => {
  try {
...
router.get("/orders/mine", requireClientAuth, async (req, res, next) => {
  try {
    const { userId } = (req as any).clientUser;
    const rows = await db
      .select({
...
router.get("/clients", requireAdminAuth, async (_req, res, next) => {
  try {
